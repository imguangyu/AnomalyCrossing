...
class rgb_r2plus1d_18_encoder(nn.Module):
    def __init__(self, freeze_backbone=False, pretrained=True, modelPath=''):
        super(rgb_r2plus1d_18_encoder, self).__init__()
        self.encoder=nn.Sequential(*list(
            r2plus1d_18( pretrained=pretrained, progress=True).children())[:-2])
...
class rgb_r2plus1d_8f_34_encoder(nn.Module):
    def __init__(self, freeze_backbone=False, pretrained=True, modelPath=''):
        ...
        self.encoder=nn.Sequential(*list(
            r2plus1d_34_8_ig65m(487, pretrained=pretrained, progress=True).children())[:-2])
...
class rgb_r2plus1d_32f_34_encoder(nn.Module):
    def __init__(self, freeze_backbone=False, pretrained=True, modelPath=''):
        ...
        self.encoder=nn.Sequential(*list(
            r2plus1d_34_32_ig65m(359, pretrained=pretrained, progress=True).children())[:-2])
