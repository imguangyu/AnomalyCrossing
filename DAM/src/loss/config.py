...
def pt_optim_init(args, model, n_data):
    ...
    if hasattr(args, "distributed") and args.distributed:
        #for ddp increase the lr by the number of processes (world size)
        lr *= args.world_size
    print("Learning rate: {}".format(lr))
    
    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), #filter out params which doesn't require grad and will not be trained
                                #model.parameters(),
                                lr=lr,
