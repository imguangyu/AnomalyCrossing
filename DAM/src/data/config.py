...
def pt_augmentation_config(args, recorder):
    if int(args.pt_spatial_size) == 112:
        ...
        resize_size = 256
    if args.method == 'ssl':
        resize_size = (args.ssl_height, args.ssl_width)
    if args.method == 'dota':
        resize_size = (args.ssl_height, args.ssl_width)
    if args.method == 'bdd':
        resize_size = (args.ssl_height, args.ssl_width)
    if args.method == 'ucf_crime':
        resize_size = (args.ssl_height, args.ssl_width)
    if args.method == 'paper_fold':
        resize_size = (args.ssl_height, args.ssl_width)
    if args.pt_mode == 'rgb':
        #note 
        if args.method in ['ssl', 'dota', 'bdd', 'ucf_crime', 'paper_fold']:
            normalize = video_transform.Normalize(mean=[0.43216, 0.394666, 0.37645], std=[0.22803, 0.22145, 0.216989]) 
            recorder.record_message('a', "image resize to " + str(resize_size))
            recorder.record_message('a', "normalization mean " + str([0.43216, 0.394666, 0.37645]) + " std " + str([0.22803, 0.22145, 0.216989]))
        else:
