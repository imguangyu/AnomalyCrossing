...
def ReadSegmentRGB(path, 
    ...
    for length_id in range(new_length):
        ...
        frame_index = frame_index % video_length
        # if frame_index >= video_length:
        #     frame_index = video_length-1
...
class DataSet(data.Dataset):
    def __init__(self, args, root_path, list_file, dataset='ucf101',
        ...
        #Get clips
        clips, lablels = make_dataset(root=self.root_path, 
    ...
    def get_dsm_items(self, index):
        path, video_length , ego_envolve, selected_cls = self.clips[index]
        # path = os.path.join(self.args.ssl_frame_path, path)

        segment_indices = random.randint(0, max(video_length - self.new_length * self.stride, 0))

        negative_segment_indices = segment_indices

        #negative 
        thresh = self.thresh
        negative_segment_indices = random.randint(0, max(video_length - self.new_length * self.stride, 0))
        if abs(negative_segment_indices - segment_indices) < thresh:
            negative_segment_indices = (negative_segment_indices + video_length // 3) % video_length
