...
def pretext_train_dota(args, recorder):
    ...
    if args.arch == 'dota':
        model, model_ema, params = pt_model_config(args, num_class)
    else:
    ...
    for epoch in range(args.pt_start_epoch, args.pt_epochs + 1):
        ...
        if hasattr(model,'module'):
            encoder_state = {k:v for k,v in model.module.state_dict().items() if k in params['state_dict_encoder'].keys()}
        else:
            encoder_state = {k:v for k,v in model.state_dict().items() if k in params['state_dict_encoder'].keys()}
        new_params['state_dict_encoder'] = encoder_state
        # state = {
        #     'opt': args,
        #     'state_dict_encoder': encoder_state,
    ...
    assert params['state_dict_encoder'].keys() == new_params['state_dict_encoder'].keys()

    torch.save(new_params, args.dota_output)
...
def train_dsm(epoch, train_loader, model, model_ema, contrast, criterion, optimizer, opt, pos_aug, neg_aug, recorder):
    ...
    for idx, (inputs, _, index) in enumerate(train_loader):
        ...
        # print (model(anchor).size())
        # print(cv2.cvtColor(numpy.asarray(anchor_old.cpu()),cv2.COLOR_RGB2BGR))
        # print(cv2.cvtColor(numpy.asarray(anchor.cpu()),cv2.COLOR_RGB2BGR))
        # import sys
        # sys.exit()
