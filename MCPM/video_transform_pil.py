...
class RandomSizedCrop:
    ...
    def __call__(self, imgmap):
        ...
        if random.random() < self.threshold: # do RandomSizedCrop
            ...
            # scale is added by ZL since we'd like to make sure 
            # we always sacle the image to a certain size
            scale = Scale(self.size, interpolation=self.interpolation)
            crop = CenterCrop(self.size)
            # return crop(imgmap)
            return crop(scale(imgmap))
...
class RandomRotation:
    def __init__(self, consistent=True, degree=15, p=1.0, resample=None):
        self.consistent = consistent
        self.degree = degree 
        self.threshold = p
        if resample != None:
    ...
    def __call__(self, imgmap):
        if random.random() < self.threshold: # do RandomRotation
            if self.consistent:
                ...
                return [i.rotate(deg, resample=self.resample, expand=True) for i in imgmap]
            else:
                return [i.rotate(np.random.randint(-self.degree, self.degree, 1)[0], resample=Image.BILINEAR, expand=True) for i in imgmap]
...
class GuassianBlur:
    def __init__(self, kernel_size=3, sigma=0.2, p=1.0):
        self.kernel_size = kernel_size
        self.sigma = sigma 
        self.threshold = p
    
    def __call__(self, imgmap):
        if random.random() < self.threshold: # do GuassianBlur
            gauss_blur = transforms.GaussianBlur( self.kernel_size, self.sigma)
            return [gauss_blur(i) for i in imgmap]
        else:
            return imgmap
