...
class Crop(object):
    """Crops the given numpy array based on the give coordinates x1 y1 x2 y2
    """
    ...
    def __call__(self, clips):
        ...
        if is_color:
            num_imgs = int(c / 3)
            cropped_clips = np.zeros((th,tw,c))
            for frame_id in range(num_imgs):
                cur_img = clips[:,:,frame_id*3:frame_id*3+3]
                crop_img = cur_img[y1:y1+th, x1:x1+tw, :]
                assert(crop_img.shape == (th, tw, 3))
                cropped_clips[:,:,frame_id*3:frame_id*3+3] = crop_img
            ...
            for frame_id in range(num_imgs):
                ...
                cropped_clips[:,:,frame_id:frame_id+1] = crop_img
            return cropped_clips
