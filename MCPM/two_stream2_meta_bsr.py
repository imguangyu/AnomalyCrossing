...
from models.classification_heads import BSR_Single
...
# parser.add_argument('--cls-head', default='bsr',
...
parser.add_argument('--shot', default=0, type=int, metavar='N',
                    help='number of shots in the few-shot learning setting')
# parser.add_argument('--query', default=0, type=int, metavar='N',
...
def main():
    ...
    # this script is only for BSR single model training
    args.cls_head = 'BSR' 
    if '3D' in args.arch:
    ...
    # note for bsr trainig no need of query samples since we are using 1st order meta training
    train_sampler = CategoriesSampler(train_dataset.labels, args.num_tasks_train, args.way, args.shot)
    val_sampler = CategoriesSampler(val_dataset.labels, args.num_tasks_val, args.way, args.shot)
...
def build_model(args):
    ...
    cls_head = BSR_Single(args.feat_dim, args.way, args.lamda)
    ...
    for i, (inputs, _) in enumerate(train_loader):
        ...
        if not args.iter_opt:
            ...
            if args.label_smoothing:
                loss, logits = cls_head.forward_loss(features, label, args.label_smoothing, args.eps)
            else:
                loss, logits = cls_head.forward_loss(features, label)
            ...
            iter_size = max(ns // nb, 1)
            loss_mini_batch_classification = 0
            totalSamplePerIter = ns
            logits_all = []
            idx = 0
            while idx < ns:
                ns_input = inputs[idx:idx+nb]
                ns_label = label[idx:idx+nb]
                if torch.cuda.is_available():
                    if HALF:
                        ns_input = ns_input.cuda().half()
                    else:
                        ns_input = ns_input.cuda()
                features = encoder(ns_input)
                if args.label_smoothing:
                    loss, logits = cls_head.forward_loss(features, ns_label, args.label_smoothing, args.eps)
                else:
                    loss, logits = cls_head.forward_loss(features, ns_label)
                loss = loss / iter_size
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(val_loader):
            ...
            if not args.iter_opt:
                ...
                logits_all = torch.cat(logits_all)
                acc_mini_batch = count_acc(logits_all, label)
                acc.update(acc_mini_batch, ns)
                lossesClassification.update(loss_mini_batch_classification, ns)
