...
def finetune(args, support_dataset, query_dataset, device):
    ...
    for inputs, target, _ in support_dataset:
        # make the node dim as: [bs(1), feat_dim(512), num_points]
...
def eveluate(args):
    ...
    fs_worset = dota_ctx.Workset(root=args.ctx_path, 
                      source=split_file, 
    ...
    for i in range(len(fs_worset)):
        ...
        support_dataset = dota_ctx.ctx_set(clips_set, 
                                     args.way,
                                     args.shot,
                                     args.query,
                                     'train',
        ...
        query_dataset = dota_ctx.ctx_set(clips_set, 
                                     args.way,
                                     args.shot,
                                     args.query,
                                     'eval',
